...
@router.post("/api/fix-overlaps")
async def api_fix_overlaps(request: Request):
    """Fix overlapping events in a single subtitle file in-place.

    Detects temporally overlapping ASS/SRT events and repositions the
    later-starting event to the top of the screen via an inline ``{\an8}``
    override. Overwrites the source file only when at least one event was
    repositioned.

    Request body (JSON):
        subtitle_path (str): Absolute path to the subtitle file.

    Returns:
        {status: "ok", repositioned: N, output_path: "..."}
    """
    import pysubs2
    ...
    try:
        ...
        from ..merge import fix_single_track_overlaps
