"""Subtitle synchronization via ffsubsync and alass."""
...
class AlassNotFoundError(SyncError):
    """alass is not installed."""
...
def _get_alass_command() -> str:
    """Return alass path or raise an error."""
    alass_path = shutil.which("alass")
    if alass_path is None:
        raise AlassNotFoundError("alass not found. Install it: cargo install alass")
    return alass_path
...
def sync_subtitles_alass(
    reference_path: str | Path,
    input_path: str | Path,
    output_path: str | Path | None = None,
) -> SyncResult:
    """Synchronize subtitles to a reference via alass.

    alass performs segment-based alignment which handles non-linear
    timing differences (e.g. different cue segmentation between
    languages).  Preferred over ffsubsync for SRT-to-SRT sync.
    ...
    The intermediate output file uses a ``.sync_tmp`` infix before
    the real extension (e.g. ``.sync_tmp.srt``) so that alass
    can detect the output format from the file extension.
    ...
    Returns:
        SyncResult with status, path to the synced file,
        detected offset, and ``engine_used="alass"``

    Raises:
        SyncError: If synchronization fails
        AlassNotFoundError: If alass is not installed
    """
    alass_cmd = _get_alass_command()
    ...
    try:
        ...
        tmp_output.unlink(missing_ok=True)
        raise SyncError("alass timeout - synchronization is taking too long") from None
    except subprocess.CalledProcessError as e:
        tmp_output.unlink(missing_ok=True)
        raise SyncError(f"alass failed:\n{e.stderr}") from e

    logger.debug(f"alass stdout:\n{result.stdout}")
    if result.stderr:
        logger.debug(f"alass stderr:\n{result.stderr}")
    ...
    if offset_ms is not None and abs(offset_ms) > 30_000:
        logger.warning(f"Large offset detected: {offset_ms}ms — verify result")
        return SyncResult(
            success=False, output_path=target_path, offset_ms=offset_ms, engine_used="alass"
        )
    ...
    return SyncResult(
        success=True, output_path=target_path, offset_ms=offset_ms, engine_used="alass"
    )
